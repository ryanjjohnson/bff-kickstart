<#--
  Forked from keycloak.v2/login/user-profile-commons.ftl with one change:
  the "username" attribute is never rendered. Users never choose their own
  username - it's generated server-side by the bff-registration-user-creation
  FormAction (providers/bff-registration-spi) as lastname_fm_#. Marking the
  "username" User Profile attribute admin-only (see realm/src/user-profile-config.json)
  is not enough to hide it here, since Keycloak still includes it in
  profile.attributes for the registration context - so we skip it explicitly.
  Everything else below is unmodified from keycloak.v2.
-->
...
<#macro userProfileFormFields>
	...
	<#list profile.attributes as attribute>
		...
		<#if attribute.name=='username'>
			<#-- suppressed: system-generated, never user-editable -->
