...
| Service | URL |
|---|---|
| Frontend (and the API, nested under it - see `docs/frontend-guide.md`'s "Backend proxying") | http://localhost:3000/bff-kickstart/ |
...
This scaffolds `src/types/<entity>.ts` and `src/features/<resource>/{api.ts, <Entity>Form.tsx, <Entity>Page.tsx, index.ts}` following the same conventions as the Facilities/Permits/Inspections features - including the module-boundary rule that other features may only import through a feature's `index.ts` (see `docs/frontend-guide.md`). It prints the remaining manual steps (add the route, the nav link, and the backend controller/entity/service) when it finishes.
...
- **Interceptors.** `src/lib/api-client.ts`'s response interceptor is what makes a revoked session
  ...
  own error-handling code for that case (see `docs/frontend-guide.md`'s "Staying signed out when
...
`docs/frontend-guide.md`'s own feature-boundary rule ("a feature may only be imported through its
